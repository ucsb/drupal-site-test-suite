...
Rebuild theme assets too if the theme has a build step (e.g. compiled CSS/JS). **Then** re-run the relevant a11y lanes (or dispatch to `drupal-accessibility-diagnostic`) and report **before → after**: findings resolved, remaining, and confirm no new violations were introduced. **Check the re-run's `summary.status` first**: if any lane comes back `incomplete`/`error` (or `summary.pages_errored > 0`), do **not** count its lower finding totals as verification; the pages that errored weren't audited. Surface the incomplete lanes and re-run before claiming the fix is verified. Bound the loop; if a finding resists fixing after 2–3 attempts, escalate to human review.
