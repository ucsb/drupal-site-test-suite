...
Follows **`drupal-coding-standards`** (Doxygen doc-blocks, translatable strings, constructor DI in the classes under test). This skill covers *writing tests*, plus a scoped verify of what it just wrote (run the new tests, lint the new files; see "Verify what you wrote"). Full-codebase QA stays with the test suite (`/tests/phpunit/run.js`, `drush utest:phpunit`) and `drupal-code-quality-audit`.
...
## Verify what you wrote (before handing over)

New test code is custom code; don't leave it for a later audit to flag. After writing:

1. **Run the new tests**: `/tests/phpunit/run.js` / `drush utest:phpunit --modules=<module>` on a suite-equipped site, or raw `vendor/bin/phpunit -c web/core <module>/tests` otherwise. They must pass (or fail only where they document a known bug you were asked to pin).
2. **Lint the new test files**: suite present → `drush utest:lint --modules=<module>`; no suite → `vendor/bin/phpcs --standard=Drupal,DrupalPractice <module>/tests` (plus `vendor/bin/phpstan analyse <module>/tests` when the project configures PHPStan). Fix what you introduced; test classes follow `drupal-coding-standards` like any other class.

Both checks read the working tree; nothing gets committed.
