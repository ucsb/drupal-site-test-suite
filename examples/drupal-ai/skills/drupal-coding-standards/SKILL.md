...
- **Accessibility of rendered output (WCAG):** `drupal-accessibility-audit` → `drupal-accessibility-remediation` (or the per-lane a11y commands: `drush utest:alfa`, `utest:axe`, `utest:pa11y`, `utest:reflow`, `utest:meta-viewport`). For accessible markup patterns, follow the `a11y` agent.
