...
Adapters emit `fix` with all-`null` fields. The **diagnostic agent** fills it using its built-in fix heuristics (and per-engine `tools/` docs when the skill ships them); the **remediation agent** acts on it.
...
Join stable parts with `:` as follows:
...
- `summary`, `surface`, `profile`, `gate`, `generated_at`, `duration_ms`, `test`, `tool` come straight from the upstream `test-suite-findings.json`.
- **`summary.status`**: full enum: `pass` | `fail` | `findings-found` | `skipped` | `incomplete` | `error`. **`incomplete` is never a pass**: the lane couldn't audit everything it tried to (see `pages_errored`), so a low finding count proves nothing. Agents must surface incomplete/errored lanes rather than reporting clean.
- **`summary.pages_errored`** (int); how many pages could not be audited (navigation error or timeout). Present only when nonzero; when present it **forces `status: incomplete`**. A11y lanes only (`null`/absent for linting).
- **`gate`**: per-lane build-gate metadata: `{ "fails_build": bool, "severities": ["critical", "serious"] }`. Every a11y lane emits `fails_build: true` with `["critical", "serious"]`; the report-only `phpunit` lane emits `fails_build: false`; the `lint` lane omits `gate` entirely (pass through as `null`). Copy verbatim; it tells the diagnostic what the suite itself gates on; don't invent gating the lane doesn't claim.
