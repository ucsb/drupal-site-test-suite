...
## Step 4 — Build `coverage` honestly (the a11y false-clean traps still apply — from a file)

A static report is only as honest as what it contains, and a11y is full of traps:

- `coverage.lanes` = per-lane `{ engine, profile_key, status, pages_tested, n_findings }` from each file.
