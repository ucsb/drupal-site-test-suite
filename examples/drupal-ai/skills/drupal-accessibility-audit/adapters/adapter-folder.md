...
- `engines_run` + `coverage.lanes` (engine, profile≈standard, status, pages_tested, n_findings); `engines_skipped` with hints. Set a per-engine `status` (`pass`/`findings-found`/`incomplete`/`error`); if pages errored or timed out during a run, record the count and mark that engine `incomplete`, never clean.
