...
Produces **one aggregated** findings envelope (`../reference/finding-format.md`) by running the project's `utest:a11y:*` lanes and merging the per-lane `test-suite-findings.json` files they emit. Default adapter whenever a reachable Drupal site with the test suite is present.
...
## Step 3 — Discover and run the **findings-emitting** lanes

Discover via `<DRUSH> list --format=json` (parse `utest:a11y:*`); don't hardcode. Then run only the lanes that **emit `test-suite-findings.json`** — verified against live output:

| Run these (emit normalized JSON) | Skip these for the audit |
| --- | --- |
| `utest:a11y:axe-full` (axe-core) | `utest:a11y:axe` — emits **HTML only**, no findings JSON |
| `utest:a11y:alfa-full` (Siteimprove Alfa) | `utest:a11y:alfa` — key-pages overlay, HTML only |
| `utest:a11y:pa11y` (pa11y-ci) | `utest:a11y:axe-watcher` / `axe-watcher-full` — need `AXE_API_KEY`; skip if absent → record in `coverage` |
| `utest:a11y:reflow` (320px, SC 1.4.10) | |
| `utest:a11y:meta-viewport` (SC 1.4.4) | |

Key flags:

- `--base-url=<BASE_URL>` (all lanes).
- `--max-pages=N` on the full/sitemap lanes — caps the crawl. Cap for speed during iteration, but **record the cap in `coverage`** (a capped run is partial coverage). `all` or `0` = no cap.
...
**Exit code:** a11y lanes **throw (exit 1) when violations are found** — verified live. That is **not** a failure; it's "findings exist." Read the JSON regardless of exit code. (This differs from `utest:lint`, which exits 0 on findings.)
...
Then read `<resolved>/<lane>/test-suite-findings.json` for each lane (`axe-full`, `alfa-full`, `pa11y`, `reflow`, `meta-viewport`). Fallback: `<repo-root>/web/sites/default/files/test-reports/<lane>/test-suite-findings.json`. A missing file for a lane you ran → that lane is **skipped** in coverage (with reason), not clean.

## Step 5 — Aggregate + normalize into ONE envelope

Validate each file's `schema_version` major (`1`), then merge:

1. **Envelope** = `suite: "accessibility"`, `source { adapter: "drush", command, base_url: <BASE_URL>, captured_at }`. Re-aggregate `summary.totals_by_severity` / `totals_by_impact` across all lanes; `pages_tested` = max across lanes (or per-lane, see coverage).
2. **`coverage`** — the honesty surface for a11y:
   - `engines_run` = the lanes that produced findings files.
   - `coverage.lanes` = per-lane detail: `{ engine, test, profile_key, status, pages_tested, n_findings }`. This is where the caveats live (see Step 6).
...
| Lane exits 1 | **violations found (normal)** | Read the JSON; it's findings, not failure. |
| No `test-suite-findings.json` for a run lane | playwright/deps failed, or you ran a key-pages overlay (`axe`/`alfa`) that emits HTML only | Mark the lane skipped with reason; use a `*-full` lane. |
| `axe-watcher*` produced nothing | no `AXE_API_KEY` | Expected — record skipped. |
