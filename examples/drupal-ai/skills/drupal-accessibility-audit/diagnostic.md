...
2. **Status gate.** Lane `status` enum: `pass` / `fail` / `findings-found` / `skipped` / `incomplete` / `error`. An `incomplete` or `error` lane is **never a clean pass**: pages it couldn't audit may hide violations. `summary.pages_errored` (present when nonzero; forces `incomplete`) says how many. Surface **which lanes were incomplete/errored and their errored-page counts** instead of reporting clean.
3. **Read `coverage.lanes` before findings:**
...
4. Only if every lane is `status: pass` and nothing was skipped, capped, incomplete, or errored, report the clean-within-coverage result; and still state the boundary.
