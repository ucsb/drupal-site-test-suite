...
```text
[input: drush per-lane a11y runs (utest:alfa/axe/pa11y/reflow/meta-viewport) | existing JSON | HTML report]
        ↓
...
On a site that ships the test suite, its own `tests/README.md` (and `tests/README_cheatsheet.md` + `tests/accessibility/config/README.md`) are the authoritative reference for the exact per-lane `utest:*` a11y commands, profiles, and report locations; defer to them for suite-specific detail.
