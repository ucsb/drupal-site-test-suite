...
2. **Read `coverage.lanes` before findings.** A11y "clean" is easy to fake:
   ...
   - Lanes that need an **API key** (axe-watcher / axe-watcher-full) or a **running site** may be skipped — surface them.
...
3. If every lane is `status: pass` **and** nothing was skipped/capped, report the clean-within-coverage result — but still state the coverage boundary.
